...
function cpu_dial(cr,cpu_usage, label, center_x, center_y)
...
label_width = label_extents.width
label_start_x = center_x - label_width / 2
label_start_y = center_y + 35
cairo_move_to(cr, label_start_x, label_start_y)
...
cpu_indicator = position_needle(cpu_usage)
...
start_angle = max_angle - math.rad(30)
end_angle = min_angle + math.rad(30)
angle_per_mark = (end_angle - start_angle) / (total_marks - 1)
mark_angle = start_angle + (mark_number - 1) * angle_per_mark
return mark_angle
end

function position_needle(cpu_usage)
min_cpu = 0
max_cpu = 100

new_target_angle = min_angle + (cpu_usage - min_cpu) / (max_cpu - min_cpu) * (max_angle - min_angle)
